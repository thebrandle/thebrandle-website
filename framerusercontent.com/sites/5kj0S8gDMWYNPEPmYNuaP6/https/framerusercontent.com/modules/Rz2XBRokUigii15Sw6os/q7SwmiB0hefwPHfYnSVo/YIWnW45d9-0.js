  ...
  (t.read = function (t) {
    ...
    (t.compare = function (t, e, r) {
      ...
      switch (t.type) {
        ...
        case x.Color:
          return I(e.type === x.Color), $(t, e);
        case x.Date:
          return I(e.type === x.Date), K(t, e);
        case x.Enum:
  ...
  tU = async (t, e) => {
    ...
    for (;;) {
      try {
        ...
        if (!tb.includes(n.status) || ++r > tw) return n;
      } catch (t) {
        if (e?.signal?.aborted || ++r > tw) throw t;
      }
      await tS(r);
    }
  };
async function tS(t) {
  let e = Math.floor(tI * (Math.random() + 1) * 2 ** (t - 1));
  await new Promise((t) => {
    setTimeout(t, e);
  });
...
var tM = class {
    async loadModel() {
      let [t] = await tk(this.options.url, [this.options.range]);
      return I(t, "Failed to load model"), tm.deserialize(t);
    }
    async getModel() {
      return (
  ...
  tF = class {
    scanItems() {
      return (
        (this.itemsPromise ??= tU(this.url).then(async (t) => {
  ...
  tN = class {
    ...
    constructor(t) {
      (this.options = t),
        ...
        c(this, "chunks"),
        (this.chunks = this.options.chunks.map((t, e) => new tF(e, t))),
        (this.schema = t.schema),
        (this.indexes = t.indexes),
        (this.resolveRichText = t.resolveRichText);
    }
  };
export { tN as DatabaseCollection, tM as DatabaseDictionaryIndex };
